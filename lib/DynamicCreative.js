class DynamicCreative {
   ...
   * @param {Array} ans
   */
  rotate_ans(canvas, ans) {
    canvas = this.rotate(canvas, ans)
    ans.map(item => {
      var temp = item.x
      item.x = item.y
      item.y = temp
   ...
   */
  toParams(canvas) {
    var ans = this.copy_sort()
    var scale = canvas.w / this.origin.w
    var last_x = canvas.w * 0.1
    var last_y = canvas.h * 0.1
    var elements_count = 0
    if (scale < 1) this.rotate(canvas, ans)
    ans.map(item => {
      if (item.type === 'Image') {
        var tempH = item.h * scale > canvas.h * 0.8 ? canvas.h * 0.8 : item.h * scale
    ...
    if (scale < 1) this.rotate_ans(canvas, ans)
    return ans
  }
}

class DynamicCreative {
  ...
  toParams(canvas) {
    ...
    ans.map(item => {
      if (item.type === 'Image') {
        if (scale > 0.6) {
          var temp_h = item.h * scale > canvas.h * max_rate ? canvas.h * max_rate : item.h * scale
          item.w = item.w * (temp_h / item.h)
          item.h = temp_h
        } else {
          var temp_w = canvas.w * 0.8
          item.h = item.h * (temp_w / item.w)
          item.w = temp_w
        }
        item.x = last_x
        item.y = last_y
        last_x = scale > 0.6 ? item.x + item.w + 10 : last_x
        last_y = scale > 0.6 ? last_y : item.h + item.y + 10
